...
class Admin:
    ...
    def manage_business(self):
        for i in b:print(i)
        modify_business = input("Please select a owner to modify his Businesses")
        if modify_business in str(available_rooms):
            for owner_key in available_rooms:
                if owner_key == modify_business:
                    print(f"The details of {modify_business} owner business is {available_rooms[modify_business]}")
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if role.split('_')[0] == 'admin':
            adm=Admin()
            while True:
                admin_tasks  = ['View Bookings','Modify Bookings','View Business','Manage Business','View Complaints']
